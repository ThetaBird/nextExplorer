  ...
  asyncHandler(async (req, res) => {
    ...
    if (payload.thumbnails && typeof payload.thumbnails === 'object') {
      ...
      if (Number.isFinite(payload.thumbnails.concurrency)) {
        updates.thumbnails.concurrency = payload.thumbnails.concurrency;
      }
      if (payload.thumbnails.showVideoCoverArt != null) {
        updates.thumbnails.showVideoCoverArt = Boolean(
          payload.thumbnails.showVideoCoverArt
        );
      }
    }
